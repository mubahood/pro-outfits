    ...
    if ($thread != null) {
        if (strlen($thread) > 2) {
            ...
            if (count($ids) > 2) {
                ...
                $product_id = $ids[2];
                $receiver = Administrator::where('id', $receiver_id)->firstOrFail();
                $product = Product::where('id', $product_id)->firstOrFail();
                $inbox = Chat::where('thread', $thread)->get();
    
                if ($product != null && $receiver != null && $sender != null) {
                    $chat_is_open = true;
                }
            }
        }
                    ...
                    <div class="message-filter">
                        ...
                        <ul class="message-list">
                            ...
                            @foreach ($threads as $item)
                                ...
                                @endphp
                                <li class="message-item {{ $is_seen }} {{ $is_active }}  "> <a
                                        href="{{ url('messages') . '/' . $item->thread }}" class="message-link">
                                        <div class="message-img active"><img src="{{ $item->product->get_thumbnail() }}"
                                                alt="avatar"></div>
                                        <div class="message-text">
                                            <h6 class="m-0 p-0 ">{{ $item->_sender->profile->username }}
                                                <span>{{ $item->created_at->diffForHumans() }}</span></h6>
                ...
                <div class="col-lg-7 col-xl-8">
                    @if (!$chat_is_open)
                        <div class="message-inbox">
                            <div class="card-body text-center">
                                ...
                                <p class="text-center mb-2">You have not selected any message. Select one message from your
                                    chat thread to start chatting!</P>
                    ...
                    @if ($chat_is_open)
                        <div class="message-inbox">
                            <div class="inbox-header">
                                <div class="inbox-header-profile">
                                    <a class="inbox-header-img rounded-0" href="#">
                                        <img class="rounded-0" src="{{ $product->get_thumbnail() }}" alt="avatar"></a>
                                    <div class="inbox-header-text">
                                        <h5 class="h6 mb-0 pb-0 " style="line-height: 18px"><a
                                                href="/{{ $product->slug }}">{{ $product->name }}</a></h5>
                                        ...
                                        <span style="line-height: 15px" class="d-block mt-0 pt-0">
                                            Chatinng with <a
                                                href="/{{ $receiver->profile->username }}">{{ $receiver->profile->company_name }}</a>
                                        </span>
                                    </div>
                                </div>
                                ...
                                @foreach ($inbox as $msg)
                                    @if ($msg->sender == $sender->id)
                                        <li class="inbox-chat-item">
                                            <div class="inbox-chat-img"><img
                                                    src="{{ URL::asset('/assets') }}/images/avatar/01.jpg" alt="avatar">
                                            </div>
                                    ...
                                    @else
                                        <li class="inbox-chat-item my-chat">
                                            <div class="inbox-chat-img"><img
                                                    src="{{ URL::asset('/assets') }}/images/avatar/02.jpg" alt="avatar">
                                            </div>
                                            <div class="inbox-chat-content">
                                                <div class="inbox-chat-text">
                                                    <p>{{ $msg->body }}</p>
                                                    ...
                                                    class="inbox-chat-time">{{ $msg->created_at->diffForHumans() }}</small>
                                            </div>
                                        </li>
                                    @endif
                                @endforeach
                            ...
                            @endphp
                            <form id="message_form" autocomplete="off" action="{{ $messgae_url }}" class="inbox-chat-form"
                                method="post">
                                @csrf
                                ...
                                <input autofocus autocomplete="off" required class="border" type="text"
                                    name="body" id="message_body" placeholder="Type a Message " /><button
